...
class GaborConv2d(nn.Module):
    ...
    # Modified in the GaborConv2d class
    def forward(self, x):
        # Logic: if in training mode (parameters changing) or kernel hasn't been generated yet (None), generate it
        ...
        # This ensures what's passed to conv2d is always a Tensor-type kernel
...
class ccnet(torch.nn.Module):
    ...
    Modification: added Center Loss central parameters and corrected ArcFace's s parameter
    '''
    def __init__(self, num_classes, weight=None):
        ...
        # [Fix] According to the description below Eq.(7) in the paper, s=64, m=0.5
        self.arclayer_ = ArcMarginProduct(2048, num_classes, s=64.0, m=0.5, easy_margin=False)

        # [New] Learnable centers for Center Loss (dimension = 4096 + 2048 = 6144)
        self.centers = nn.Parameter(torch.randn(num_classes, 6144))
    ...
    def load_pretrained_weights(self, path):
        if os.path.exists(path):
            ...
            pretrained_dict = {k: v for k, v in pretrained_dict.items()
                            if k in model_dict and "arclayer_" not in k and "centers" not in k}
    ...
    def forward(self, x, y=None):
        ...
        x1 = self.fc(x)
        x = self.fc1(x1)
        fe = torch.cat((x1, x), dim=1)  # Feature concatenation, total dimension 6144

        # Normalize features for contrastive loss, triplet loss, and center loss
        norm_fe = F.normalize(fe, dim=-1)

        x = self.drop(x)
        logits = self.arclayer_(x, y)

        return logits, norm_fe

    # [New] Compute Center Loss
    def get_center_loss(self, features, labels):
        batch_size = features.size(0)
        centers_batch = self.centers.index_select(0, labels)
        # Eq.(8): Compute squared L2 distance between features and corresponding class centers
