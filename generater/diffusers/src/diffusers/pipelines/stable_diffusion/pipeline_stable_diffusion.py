...
class StableDiffusionPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromSingleFileMixin):
    ...
    def prepare_latents(self, batch_size, num_channels_latents, height, width, dtype, device, generator, latents=None, timestep=None):
        ...
        if latents is None:
            ...
            # add real image initialized latents
            ...
            init_latents = self.vae.config.scaling_factor * latents
            noise = randn_tensor(shape, generator=generator, device=device, dtype=dtype)
            latents = self.scheduler.add_noise(init_latents, noise, timestep)
            hist = torch.histc(latents, bins=100, min=0, max=0).long()
            from time import time
            fname = f'hist_{int(time())}.pt'
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred = self.unet(
                    ...
                    return_dict=False,
                    controlnet_cond_guidance=controlnet_cond,
                    controlnet_interpolator=controlnet_interpolator,
                )[0]
